...
public class EncuestaSintomas extends AppCompatActivity {

    private int tipo_encuesta;
    private RegistroEncuesta mRegEncuesta;
    private List<String> mSintomas;

    @OnClick(R.id.b_confirmar_sintomas) void OnClick(){
        Intent intent;
        intent = new Intent(this, EncuestaEstadoFisico.class);
        setContentView(R.layout.activity_encuesta_estado_fisico);
        mSintomas = new ArrayList<>();
        mSintomas.add("prueba1");
        mSintomas.add("prueba2");
        mRegEncuesta.setActividades(mSintomas);
        if(tipo_encuesta == 1) {
